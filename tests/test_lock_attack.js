...
async function runTests() {
  // WorldManager đã tự khởi tạo quái trên tất cả map trong constructor
  ...
  worldManager.maps[1].monsters.push(monNear);
  ...
  worldManager.maps[1].monsters.push(monFar, monClose);
...
runTests().then(() => {
  ...
  if (worldManager.maps[1]) {
    worldManager.maps[1].monsters = worldManager.maps[1].monsters.filter(m => m.id < 999900);
  }
  process.exit(0);
}).catch(err => {
